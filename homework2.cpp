...
void task6() {                                                                                     // Задание 6: "Ссуда".
    ...
    bool found = false;                                         //p = 0.1;
    for (p = 0.1; p <= 100; p += 0.1) {                         //while (p <= 100 && !found)       // Перебираем возможные проценты от 0.1% до 100%.
        float r = p / 100;
        float numerator = S * r * pow(1 + r, n);
        float denominator = 12 * (pow(1 + r, n) - 1);
        float calculated_m = numerator / denominator;

        if (fabs(calculated_m - m) < 0.1) {                                                        // Если рассчитанный платеж отличается
            cout << "Процент p: " << p << "%" << endl;                                             // от заданного менее чем на 1 рубль,
            found = true;                                       
            break;                                              //p += 0.1                         // считаем что нашли подходящую ставку.
        }
    }

    if (!found) {
        cout << "Подходящий процент не найден в диапазоне 0.1-100%" << endl;
    }
}
